...
下圖是一個多分支的例子。`main` 分支（在 Git 中，我們有時也會用
`master` 來表示主分支）是帶有最新可用功能的分支，`feature-1` 和 `feature-2`
...
Git 中，我們會利用引用來指向某個特定的提交。比如，分支實際上就是引用 (reference/ref)。`main`
實際上就是一個指向 `main` 分支中最新的提交的引用。其他的分支亦是如此。你可以將引用理解為如
...
請注意，本地和遠程倉庫之間是**相對**的關係。在大多數情況下，遠程倉庫是一個服務器。但是，遠程倉庫也可能是另一個用戶的本地倉庫。比如，Alice
和 Bob 在同一個項目上工作。他們互相將對方的倉庫設置為遠程倉庫。在這種情況下，Alice 的遠程倉庫是 Bob 的本地倉庫，反之亦然。

每個倉庫都可以設置多個遠程倉庫。這些遠程倉庫至少應當和本地倉庫有共同的初次提交（初始提交）。如果沒有，你需要強制推送或者拉取來覆蓋。
...
是遠程倉庫的名字，`branch` 是遠程分支的名字。比如，`origin/main` 是遠程倉庫
`origin` 的 `main` 分支。
...
你可以將本文用於任何目的，只要你在第一處使用的地方明確標註原作者和鏈接
