...
在合併分支時，如果雙方修改了同一部分文件，可能會發生衝突。我們將在本文後面討論如何解決衝突。
...
- 提交之間的關係複雜，不夠清晰。
- 當合併的分支有大量提交時，衝突解決過程可能會複雜，與變基的情況相比。這是因為變基過程會逐個應用提交，因此更細粒度。

### 變基 (Rebase)

變基與合併有所不同。如果提交關係如下圖所示：
...
然後，將 `master` 分支變基到 `feat` 分支後，圖表將如下所示：
...
`feat` 分支已經變基到了 `master` 分支，提交 `C`、`E`、`G` 已經應用到了
`master` 分支的最新提交上。`C1`、`E1`、`G1` 是變基操作創建的新提交。本質上，變基是將原來的基底從公共祖先變成了要變基的分支。

在實際操作中，rebase 操作是將所有受影響的分支（包括從公共祖先到當前分支的所有分支）逐個應用到要變基的分支上。這就是為什麼它被稱為變基，因為它將當前分支的基底從公共祖先設置為要變基的分支。

請注意，你應該是要變基的分支的唯一維護者。否則，你可能會遇到很多問題。
...
> 例如，如果你在 `feat` 分支上，想要變基到 `master` 分支，你可以使用以下命令：
>
> ```bash
> git rebase master
> ```
...
- `<commit>` 參數可以是提交哈希或任何提交的引用。
- 如果沒有指定提交，Git 將變基當前分支的遠程跟踪分支。
- 如果發生衝突，你應該解決衝突，然後使用 `git rebase --continue` 繼續變基操作。我們將在本文後面討論如何解決衝突。
...
- 如果要變基的提交與當前分支有很大差異，衝突解決會更容易，因為變基過程會逐個應用提交。

缺點：

- 如果要變基的提交的提交者不是執行變基操作的人，那麼這些提交將失去原始提交者（以及簽名）。
...
你可以使用 `git status` 命令查看合併的狀態。在有衝突的文件中，你會看到類似以下消息：
...
解決衝突後，你應該根據你的合併策略使用以下命令：

- 對於合併策略，添加解決的文件（例如，`git add <file>`），然後創建一個新的提交（例如，`git commit`）。
- 對於變基策略，添加解決的文件（例如，`git add <file>`），然後繼續變基操作（例如，`git rebase --continue`）。
...
- 變基策略：在 `feat` 分支上變基 `master` 分支，然後切換到 `master`
  分支，並以快進合併的方式將 `feat` 分支合併。

## 相對引用

當你在多個分支上工作時，相較於使用哈希值，提交的相對關係更有意義。有時，用某個提交的相對關係來表達另一個提交會更加方便且明確（如
`HEAD` 的父提交）。
...
可能會因為當前分支的更改（這種情況只會在當前分支和另一個分支的文件不同的情況下發生）而拒絕切換。
...
如果你誤刪了已提交的文件或目錄，你可以使用以下命令將其恢復到工作目錄：

```bash
git restore <path>...
```

請注意，這個操作會丟棄所有已更改的文件，因此可能會有風險。為了更安全地實現類似的功能，你也可以[暫時儲藏更改](#暫時儲藏-stash)。

> 例如，如果你誤刪了 `README.txt` 文件，你可以使用以下命令將其恢復：
>
> ```bash
> git restore README.txt
> ```

如果你想恢復的文件源不是 `HEAD`，你可以使用 `--source=<tree>` 選項指定。`<tree>` 可以是提交哈希或任何提交的引用。例如，如果你想從 `HEAD` 的父提交中恢復 `README.txt` 文件，你可以使用以下命令：

```bash
git restore --source=HEAD~ README.txt
```

如果你的 Git 版本不支持 `git restore` 命令，你可以使用 `git checkout` 命令代替。用法與 `git restore` 命令類似：

```bash
git checkout <path>...
```

> 例如，如果你在前面的例子中誤刪了 `README.txt` 文件，你可以使用以下命令：
>
> ```bash
...
如果你誤將文件添加到暫存區，你可以使用以下命令取消暫存：

```bash
git restore --staged <path>...
```

> 例如，如果你誤將 `README.txt` 文件添加到暫存區，你可以使用以下命令取消暫存：
>
> ```bash
...
> 例如，如果你在前面的例子中誤將 `README.txt` 文件添加到暫存區，你可以使用以下命令：
>
> ```bash
...
- `--soft`：不會對暫存的文件或工作目錄進行任何更改（但是會將 HEAD 重置為
  `<commit>`，就像所有模式一樣）。這將使所有更改的文件變為待提交文件。
- `--mixed`（默認）：重置暫存區但不重置工作目錄（即更改的文件會被保留，但不會標記為提交），並報告未更新的文件。
- `--hard`：重置暫存區和工作目錄。自 `<commit>` 以來對工作目錄中已跟踪文件的任何更改都將被丟棄。任何在寫入任何已跟踪文件的路徑上的未跟踪文件或目錄都將被直接刪除。
- `--merge`：重置暫存區並更新工作目錄中 `<commit>` 和 `HEAD`
  之間不同的文件，但保留暫存區和工作目錄之間不同的文件（即具有未添加的更改的文件）。如果
  `<commit>` 和暫存區之間的文件之間有不同的文件具有未提交的更改，操作將被中止。
- `--keep`：重置暫存區的內容並更新工作目錄中 `<commit>` 和 `HEAD`
  之間不同的文件，並將其餘的文件（包括暫存的和未暫存的）當作未暫存[^1]。如果
  `<commit>` 和 `HEAD` 之間的文件之間有未提交的更改，重置將被中止。

[^1]: 原文中沒有直接提及所有其他文件（包括暫存或未暫存的）都被視為未暫存。這部分是為了使解釋更清晰而添加的。
...
要顯示某些文件或目錄的差異，你可以指定它們：

```bash
git diff <commit> <path>...
```

> 例如，如果你想顯示 `README.txt` 文件的工作目錄和 `feat`
> 分支之間的差異，你可以使用以下命令：
>
> ```bash
...
要為任何差異生成補丁，你只需要將前面的情況的輸出重定向到一個文件。

> 例如，如果你想為當前工作目錄和 `HEAD` 之間的差異生成一個補丁文件 `patch.diff`，你可以使用以下命令：
>
> ```bash
> git diff > patch.diff
> ```
...
如果你想在文件中查找特定內容（例如，一個字符串），你可以使用以下命令：
...
> 注意：此命令有其他替代方案（例如 [rip-grep][ripgrep]），我們將在接下來的文章中介紹。
...
| `git remote show` | 列出遠程倉庫 | 使用 `-v` 顯示更多細節；你可以通過名稱指定倉庫 |
...
| `git rebase <branch>` | 變基分支 | 使用 `-i` 或 `--interactive` 使用更多功能 |
...
| `git restore <path>` | 從提交中恢復文件到工作目錄 | 使用 `--source=<tree>` 指定源；另一種解決方案：`git checkout <path>` |
...
| `git grep <pattern> <path>...` | 在文件中查找特定內容 | 如果未指定路徑，將在整個倉庫中搜索 |
| `git reflog` | 顯示引用日志 | |

## 總結

在本文中，我們介紹了 Git 的高級用法，重點介紹了與合併分支相關的命令。希望這篇文章對你在管理多個分支方面有很大幫助。我們將在下一篇文章中介紹子模塊的用法。
